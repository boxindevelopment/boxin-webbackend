...
$(function() {
    ...
    function action(id){
        var $action = '<a class="btn btn-info btn-sm" href="{{route('add-item.index')}}/' + id + '/edit" title="Edit" style="margin-right:5px;"><i class="fa fa-pencil"></i></a>';
        return $action;
    }
    ...
    var $table = $('#table-data1').dataTable( {
        ...
        "columnDefs": [
            ...
            { "name": "items", "targets": 4 },
            { "name": "types_of_pickup_id", "sClass": "center", "targets": 5 },
            { "name": "status_name", "sClass": "center",  "targets": 6 },
        ],
        "ajax": {
            "url": "{{ route('add-item.ajax') }}",
            "type": "POST",
            "data": function ( d ) {
