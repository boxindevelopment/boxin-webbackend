...
$(function() {
    ...
    function action(id){
        var $action = '<form action="{{route('order.cancel.index')}}' + id + '" method="post" style="margin-top:5px;">';
                $action += '@csrf';
                $action += '@method('DELETE')';
                $action += '<a class="btn btn-primary btn-sm" href="{{route('order.cancel.index')}}/order-detail/' + id + '" title="View Detail" style="margin-right:5px;"><i class="fa fa-eye"></i></a>';
