...
class AddItemBoxPaymentRepository implements AddItemBoxPaymentRepositoryInterface
{
    ...
    public function getCount($args = [])
    {
        ...
        $query->leftJoin('order_details','order_details.id','=','add_item_box_payments.order_detail_id');
        $query->leftJoin('users','users.id','=','add_item_box_payments.user_id');
        $query->leftJoin('status','status.id','=','add_item_box_payments.status_id');
        if(Auth::user()->roles_id == 2){
            $query->leftJoin('orders','orders.id','=','order_details.order_id');
            $query->where('orders.area_id', $admin->area_id);
        }
        $query->where('add_item_box_payments.id_name', 'like', '%'.$args['searchValue'].'%');
        return $query->count();
    }
    ...
    public function getData($args = [])
    {
        ...
        $query = $this->model->query();
        $query->select('add_item_box_payments.*', 'users.first_name',  'users.last_name', 'status.name as status_name');
        $query->leftJoin('order_details','order_details.id','=','add_item_box_payments.order_detail_id');
        $query->leftJoin('users','users.id','=','add_item_box_payments.user_id');
        $query->leftJoin('status','status.id','=','add_item_box_payments.status_id');
        if(Auth::user()->roles_id == 2){
